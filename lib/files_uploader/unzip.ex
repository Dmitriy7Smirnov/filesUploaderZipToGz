defmodule Unzip do
  ...
  def test() do
    x = "WOW"
    IO.inspect(x, label: "MODULE UNZIP FUNCTION TEST")
  end
  ...
  def file_stream!(%Unzip{zip: zip, cd_list: cd_list}, file_name) do
    ...
    IO.inspect(local_header, label: "MODULE UNZIP FUNCTION FILE_STREAM VAR LOCAL_HEADER")
  ...
  defp stream!(file, offset, size) do
    ...
    x = "WOW"
    IO.inspect(x, label: "MODULE UNZIP FUNCTION STREAM! ")
  ...
  defp decompress(stream, 0x8, uncompressed_size) do
    ...
    |> Stream.transform(
      ...
      fn {z, _flag} ->
        IO.inspect(x, label: "MODULE UNZIP FUNCTION DECOMPRESS FINISHED")
        :zlib.inflateReset(z)
        :zlib.close(z)
      end
    )
  ...
  defp crc_check(stream, expected_crc) do
    IO.inspect(expected_crc, label: "MODULE UNZIP FUNCTION CRC_CHECK")
    stream
    |> Stream.transform(
      fn -> :erlang.crc32(<<>>) end,
